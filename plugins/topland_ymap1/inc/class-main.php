...
public function __construct( $file ){
	...
	// Model
	self::$model = new ToplandYmapModel();
	...
	// Shortcodes
    add_shortcode('topland_ymap', array(__CLASS__, 'replace_shortcode') );
}
...
function activate(){
	...
	if ($wpdb->get_var("SHOW TABLES LIKE '" . TOPLAND_YMAP_DB_TABLE_NAME . "'") != TOPLAND_YMAP_DB_TABLE_NAME)
	{
		$sql = "CREATE TABLE " . TOPLAND_YMAP_DB_TABLE_NAME . " (
		...
		// Назначение dbDelta: создание и обновление таблицы
      	...
      	add_option(TOPLAND_YMAP_PLUGIN_NAME . "_db_version", TOPLAND_YMAP_PLUGIN_DB_VERSION);
	}
}
...
static function register_plugin_button_in_admin_menu(){
	...
	add_menu_page(
		TOPLAND_YMAP_PLUGIN_NAME_RU, 						// содержимое <title>
		TOPLAND_YMAP_PLUGIN_NAME_RU,							// название пункта в меню
		'manage_options',									// уровень доступа (взял из примера)
		TOPLAND_YMAP_PLUGIN_NAME,							// URL страницы с плагином
		array(__CLASS__, 'render_admin_page'), 				// функция, генерирующая страницу
		plugins_url( TOPLAND_YMAP_PLUGIN_NAME . '/static/images/admin-menu-button.png' ) // адрес иконки
	);
}
...
function routing_handlers(){
	if (self::is_this_plugin_admin_page()) {
		...
		case 'add':
			...
			self::$model->save();
			print('<script>window.location = "' . TOPLAND_YMAP_PLUGIN_ADMIN_URL . '"</script>');
			break;
		case 'edit':
			self::$model->get( $id );
			self::$model->name 	  = $name;
...
static function enqueue_static() {
	wp_enqueue_style(TOPLAND_YMAP_PLUGIN_NAME . '_style', plugins_url( TOPLAND_YMAP_PLUGIN_NAME . '/static/css/style.css' ));
}
...
static function media_selector_print_scripts() {
	$my_saved_attachment_post_id = get_option( TOPLAND_YMAP_PLUGIN_NAME.'_media_selector_attachment_id', 0 );
...
static function is_this_plugin_admin_page() {
	return is_admin() && ((isset($_GET['page'])) && ($_GET['page'] == TOPLAND_YMAP_PLUGIN_NAME));
}
