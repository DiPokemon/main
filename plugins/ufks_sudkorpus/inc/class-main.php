...
public function __construct( $file ){
	...
	// Model
	self::$model = new UfksSudkorpusModel();
	...
	// Shortcodes
    add_shortcode('handball161rus_sudkorpus', array(__CLASS__, 'replace_shortcode') );
}
...
function activate(){
	...
	if ($wpdb->get_var("SHOW TABLES LIKE '" . UFKS_SUDKORPUS_DB_TABLE_NAME . "'") != UFKS_SUDKORPUS_DB_TABLE_NAME)
	{
		$sql = "CREATE TABLE " . UFKS_SUDKORPUS_DB_TABLE_NAME . " (
			id mediumint(9) NOT NULL AUTO_INCREMENT,
			order mediumint(9) NULL,
			fullname tinytext NULL,
			job_position tinytext NULL,
			birth_date tinytext NULL,
		...
		// Назначение dbDelta: создание и обновление таблицы
      	...
      	add_option(UFKS_SUDKORPUS_PLUGIN_NAME . "_db_version", UFKS_SUDKORPUS_PLUGIN_DB_VERSION);
	}
}
...
static function register_plugin_button_in_admin_menu(){
	...
	add_menu_page(
		UFKS_SUDKORPUS_PLUGIN_NAME_RU, 						// содержимое <title>
		UFKS_SUDKORPUS_PLUGIN_NAME_RU,							// название пункта в меню
		'manage_options',										// уровень доступа (взял из примера)
		UFKS_SUDKORPUS_PLUGIN_NAME,							// URL страницы с плагином
		array(__CLASS__, 'render_admin_page'), 					// функция, генерирующая страницу
		plugins_url( UFKS_SUDKORPUS_PLUGIN_NAME . '/static/images/admin-menu-button.png' ) // адрес иконки
	);
}
...
static function render_admin_page(){
	if (is_admin()) {
		if ( (isset($_GET['page'])) && ($_GET['page'] == UFKS_SUDKORPUS_PLUGIN_NAME) ) {
...
function routing_handlers(){
	if (is_admin()) {
		if ( (isset($_GET['page'])) && ($_GET['page'] == UFKS_SUDKORPUS_PLUGIN_NAME) ) {
			...
			if (isset($_POST['data_fullname']))
				$fullname = $_POST['data_fullname'];
			
			if (isset($_POST['data_job_position']))
				$job_position = $_POST['data_job_position'];

			if (isset($_POST['data_birth_date']))
				$birth_date = $_POST['data_birth_date'];
			...
			case 'add':
				...
				self::$model->job_position  = $job_position;
				self::$model->birth_date 		= $birth_date;
				self::$model->image_attachment_id = $image_attachment_id;
				self::$model->save();
				print('<script>window.location = "' . UFKS_SUDKORPUS_PLUGIN_ADMIN_URL . '"</script>');
				break;
			case 'edit':
				self::$model->get( $id );
				self::$model->order 		= $order;
...
static function enqueue_static() {
	wp_enqueue_style(UFKS_SUDKORPUS_PLUGIN_NAME . '_style', plugins_url( UFKS_SUDKORPUS_PLUGIN_NAME . '/static/css/style.css' ));
}
 ...
 */
	...
	static function enqueue_plugin_scripts($plugin_array)
	{
	    ...
	    $plugin_array["sudkorpus_button_plugin"] = plugins_url( UFKS_SUDKORPUS_PLUGIN_NAME . '/static/js/modify_tiny_mce.js' );
	    return $plugin_array;
	}
	/**
	...
	static function register_buttons_editor($buttons)
	{
	    ...
	    array_push($buttons, "sudkorpus_tinymce_button");
	    return $buttons;
	}
...
static function media_selector_print_scripts() {
	$my_saved_attachment_post_id = get_option( UFKS_SUDKORPUS_PLUGIN_NAME.'media_selector_attachment_id', 0 );
...
static function is_this_plugin_admin_page() {
	return is_admin() && ((isset($_GET['page'])) && ($_GET['page'] == UFKS_SUDKORPUS_PLUGIN_NAME));
}
