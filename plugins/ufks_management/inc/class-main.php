...
public function __construct( $file ){
	...
	// Model
	self::$model = new UfksManagementModel();
	...
	// Shortcodes
    add_shortcode('handball161rus_management', array(__CLASS__, 'replace_shortcode') );
}
...
function activate(){
	...
	if ($wpdb->get_var("SHOW TABLES LIKE '" . UFKS_MANAGEMENT_DB_TABLE_NAME . "'") != UFKS_MANAGEMENT_DB_TABLE_NAME)
	{
		$sql = "CREATE TABLE " . UFKS_MANAGEMENT_DB_TABLE_NAME . " (
		...
		// Назначение dbDelta: создание и обновление таблицы
      	...
      	add_option(UFKS_MANAGEMENT_PLUGIN_NAME . "_db_version", UFKS_MANAGEMENT_PLUGIN_DB_VERSION);
	}
}
...
static function register_plugin_button_in_admin_menu(){
	...
	add_menu_page(
		UFKS_MANAGEMENT_PLUGIN_NAME_RU, 						// содержимое <title>
		UFKS_MANAGEMENT_PLUGIN_NAME_RU,							// название пункта в меню
		'manage_options',										// уровень доступа (взял из примера)
		UFKS_MANAGEMENT_PLUGIN_NAME,							// URL страницы с плагином
		array(__CLASS__, 'render_admin_page'), 					// функция, генерирующая страницу
		plugins_url( UFKS_MANAGEMENT_PLUGIN_NAME . '/static/images/admin-menu-button.png' ) // адрес иконки
	);
}
...
static function render_admin_page(){
	if (is_admin()) {
		if ( (isset($_GET['page'])) && ($_GET['page'] == UFKS_MANAGEMENT_PLUGIN_NAME) ) {
	...
	if (is_admin()) {
		if ( (isset($_GET['page'])) && ($_GET['page'] == UFKS_MANAGEMENT_PLUGIN_NAME) ) {
			...
			case 'add':
				...
				self::$model->save();
				print('<script>window.location = "' . UFKS_MANAGEMENT_PLUGIN_ADMIN_URL . '"</script>');
				break;
			case 'edit':
				self::$model->get( $id );
				self::$model->fullname 		= $fullname;
...
static function enqueue_static() {
	wp_enqueue_style(UFKS_MANAGEMENT_PLUGIN_NAME . '_style', plugins_url( UFKS_MANAGEMENT_PLUGIN_NAME . '/static/css/style.css' ));
}
 ...
 */
	...
	static function enqueue_plugin_scripts($plugin_array)
	{
	    ...
	    $plugin_array["management_button_plugin"] = plugins_url( UFKS_MANAGEMENT_PLUGIN_NAME . '/static/js/modify_tiny_mce.js' );
	    return $plugin_array;
	}
	/**
...
static function media_selector_print_scripts() {
	$my_saved_attachment_post_id = get_option( UFKS_MANAGEMENT_PLUGIN_NAME.'media_selector_attachment_id', 0 );
...
static function is_this_plugin_admin_page() {
	return is_admin() && ((isset($_GET['page'])) && ($_GET['page'] == UFKS_MANAGEMENT_PLUGIN_NAME));
}
