...
$form_action = UFKS_MANAGEMENT_PLUGIN_ADMIN_URL . '&action=add';

if ($mode == 'edit')
	$form_action = UFKS_MANAGEMENT_PLUGIN_ADMIN_URL . '&action=edit';


update_option(
	UFKS_MANAGEMENT_PLUGIN_NAME.'_media_selector_attachment_id',
...
<div class="wrap">
	...
	<a href="<?= UFKS_MANAGEMENT_PLUGIN_ADMIN_URL ?>" class="page-title-action">← Назад</a>
				...
				<tr>
					...
					<td>
						<div class='image-preview-wrapper'>
							<img id='image-preview' src='<?php echo self::$model->get_image_attachment_filepath( get_option( UFKS_MANAGEMENT_PLUGIN_NAME.'_media_selector_attachment_id' ) ); ?>' height='100'>
						</div>
						<input id="upload_image_button" type="button" class="button" value="Выбрать изображение" />
						<input type='hidden' name='data_image_attachment_id' id='image_attachment_id' value='<?php echo get_option( UFKS_MANAGEMENT_PLUGIN_NAME.'_media_selector_attachment_id' ); ?>'>
					</td>	
				</tr>
			</tbody>
		</table>
