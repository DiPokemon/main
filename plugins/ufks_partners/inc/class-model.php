...
public function get($id){
	...
	$query = "SELECT * FROM `" . UFKS_PARTNERS_DB_TABLE_NAME . "` WHERE id = '" . $id . "' LIMIT 1";
...
public function get_list(){
	...
	$query = "SELECT * FROM `" . UFKS_PARTNERS_DB_TABLE_NAME . "`";
...
public function get_image_attachment_filepath($image_attachment_id){
	if (is_null($image_attachment_id) || empty($image_attachment_id) || ($image_attachment_id == 0))
		return plugins_url( UFKS_PARTNERS_PLUGIN_NAME . '/static/images/no-avatar.png' ) ;
	else
