...
public function __construct( $file ){
	...
	// Model
	self::$model = new UfksPartnersModel();
	...
	// Shortcodes
    add_shortcode('ufks_partners', array(__CLASS__, 'replace_shortcode') );
}
...
function activate(){
	...
	if ($wpdb->get_var("SHOW TABLES LIKE '" . UFKS_PARTNERS_DB_TABLE_NAME . "'") != UFKS_PARTNERS_DB_TABLE_NAME)
	{
		$sql = "CREATE TABLE " . UFKS_PARTNERS_DB_TABLE_NAME . " (
			id mediumint(9) NOT NULL AUTO_INCREMENT,
			time bigint(11) DEFAULT '0' NOT NULL,
			name tinytext NOT NULL,
			image_attachment_id int(11),
			url text NOT NULL,
			UNIQUE KEY id (id)
		);";
		...
		// Назначение dbDelta: создание и обновление таблицы
      	...
      	add_option(UFKS_PARTNERS_PLUGIN_NAME . "_db_version", UFKS_PARTNERS_PLUGIN_DB_VERSION);
	}
}
...
static function register_plugin_button_in_admin_menu(){
	...
	add_menu_page(
		UFKS_PARTNERS_PLUGIN_NAME_RU, 						// содержимое <title>
		UFKS_PARTNERS_PLUGIN_NAME_RU,						// название пункта в меню
		'manage_options',									// уровень доступа (взял из примера)
		UFKS_PARTNERS_PLUGIN_NAME,							// URL страницы с плагином
		array(__CLASS__, 'render_admin_page'), 				// функция, генерирующая страницу
		plugins_url( UFKS_PARTNERS_PLUGIN_NAME . '/static/images/admin-menu-button.png' ) // адрес иконки
	);
}
...
function routing_handlers(){
	if (self::is_this_plugin_admin_page()) {
		...
		case 'add':
			...
			self::$model->save();
			print('<script>window.location = "' . UFKS_PARTNERS_PLUGIN_ADMIN_URL . '"</script>');
			break;
		case 'edit':
			self::$model->get( $id );
			self::$model->name 	  = $name;
...
static function enqueue_static() {
	wp_enqueue_style(UFKS_PARTNERS_PLUGIN_NAME . '_style', plugins_url( UFKS_PARTNERS_PLUGIN_NAME . '/static/css/style.css' ));
}
...
static function media_selector_print_scripts() {
	$my_saved_attachment_post_id = get_option( UFKS_PARTNERS_PLUGIN_NAME.'media_selector_attachment_id', 0 );
...
static function is_this_plugin_admin_page() {
	return is_admin() && ((isset($_GET['page'])) && ($_GET['page'] == UFKS_PARTNERS_PLUGIN_NAME));
}
