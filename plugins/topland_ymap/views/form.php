...
$form_action = TOPLAND_YMAP_PLUGIN_ADMIN_URL . '&action=add';

if ($mode == 'edit')
	$form_action = TOPLAND_YMAP_PLUGIN_ADMIN_URL . '&action=edit';

update_option( 
	TOPLAND_YMAP_PLUGIN_NAME.'_media_selector_attachment_id',
...
<div class="wrap">
	<h1 class="wp-heading-inline"><?= $form_title ?> социальную кнопку</h1>
	<a href="<?= TOPLAND_YMAP_PLUGIN_ADMIN_URL ?>" class="page-title-action">← Назад</a>
			...
			<tr>
				...
				<td>
					<div class='image-preview-wrapper'>
						<img id='image-preview' src='<?php echo self::$model->get_image_attachment_filepath(get_option( TOPLAND_YMAP_PLUGIN_NAME.'_media_selector_attachment_id' )) ?>' height='32'>
					</div>
					<input id="upload_image_button" type="button" class="button" value="Выбрать изображение" />
					<input type='hidden' name='data_image_attachment_id' id='image_attachment_id' value='<?php echo get_option( TOPLAND_YMAP_PLUGIN_NAME.'_media_selector_attachment_id' ); ?>'>
				</td>	
			</tr>
		</tbody>
		</table>
