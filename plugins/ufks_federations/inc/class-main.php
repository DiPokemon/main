...
public function __construct( $file ){
	...
	// Model
	self::$model = new UfksFederationsModel();
	...
	if (is_admin()) {
		...
		if (self::is_this_plugin_admin_page()) {
			...
			// 	// Замечание: wp_ajax_(action_name)
	...
	// Shortcodes
    add_shortcode('handball161rus_federations', array(__CLASS__, 'replace_shortcode') );
}
...
function activate(){
	...
	if ($wpdb->get_var("SHOW TABLES LIKE '" . UFKS_FEDERATIONS_DB_TABLE_NAME . "'") != UFKS_FEDERATIONS_DB_TABLE_NAME)
	{
		$sql = "CREATE TABLE " . UFKS_FEDERATIONS_DB_TABLE_NAME . " (
			id int(11) NOT NULL AUTO_INCREMENT,
			name tinytext NULL,
			address tinytext NULL,
			phone tinytext NULL,
			email tinytext NULL,
			site_url tinytext NULL,
			chief_fullname tinytext NULL,
		...
		// Назначение dbDelta: создание и обновление таблицы
      	...
      	add_option(UFKS_FEDERATIONS_PLUGIN_NAME . "_db_version", UFKS_FEDERATIONS_PLUGIN_DB_VERSION);
	}
}
...
static function register_plugin_button_in_admin_menu(){
	...
	add_menu_page(
		UFKS_FEDERATIONS_PLUGIN_NAME_RU, 						// содержимое <title>
		UFKS_FEDERATIONS_PLUGIN_NAME_RU,						// название пункта в меню
		'manage_options',										// уровень доступа (взял из примера)
		UFKS_FEDERATIONS_PLUGIN_NAME,							// URL страницы с плагином
		array(__CLASS__, 'render_admin_page'), 					// функция, генерирующая страницу
		plugins_url( UFKS_FEDERATIONS_PLUGIN_NAME . '/static/images/admin-menu-button.png' ) // адрес иконки
	);
}
...
static function render_admin_page(){
	if (is_admin()) {
		if ( (isset($_GET['page'])) && ($_GET['page'] == UFKS_FEDERATIONS_PLUGIN_NAME) ) {
...
function routing_handlers(){
	if (is_admin()) {
		if ( (isset($_GET['page'])) && ($_GET['page'] == UFKS_FEDERATIONS_PLUGIN_NAME) ) {
			...
			if (isset($_POST['data_site_url']))
				$site_url = $_POST['data_site_url'];

			if (!empty($site_url))
				if (!stristr($site_url, 'http://') && !stristr($site_url, 'https://'))
					$site_url = 'http://'.$site_url;

			if (isset($_POST['data_chief_fullname']))
				$chief_fullname = $_POST['data_chief_fullname'];
			...
			case 'add':
				...
				self::$model->site_url = $site_url;
				self::$model->chief_fullname = $chief_fullname;
				self::$model->image_attachment_id = $image_attachment_id;
				self::$model->save();
				print('<script>window.location = "' . UFKS_FEDERATIONS_PLUGIN_ADMIN_URL . '"</script>');
				break;
			case 'edit':
				self::$model->get( $id );
				self::$model->name 	  = $name;
...
static function enqueue_static() {
	wp_enqueue_style(UFKS_FEDERATIONS_PLUGIN_NAME . '_style', plugins_url( UFKS_FEDERATIONS_PLUGIN_NAME . '/static/css/style.css' ));
}
 ...
 */
	...
	static function enqueue_plugin_scripts($plugin_array)
	{
	    ...
	    $plugin_array["federations_button_plugin"] = plugins_url( UFKS_FEDERATIONS_PLUGIN_NAME . '/static/js/modify_tiny_mce.js' );
	    return $plugin_array;
	}
	/**
	...
	static function register_buttons_editor($buttons)
	{
	    ...
	    array_push($buttons, "federations_tinymce_button");
	    return $buttons;
	}
...
static function media_selector_print_scripts() {
	$my_saved_attachment_post_id = get_option( UFKS_FEDERATIONS_PLUGIN_ADMIN_URL.'media_selector_attachment_id', 0 );
...
static function is_this_plugin_admin_page() {
	return is_admin() && ((isset($_GET['page'])) && ($_GET['page'] == UFKS_FEDERATIONS_PLUGIN_NAME));
}
